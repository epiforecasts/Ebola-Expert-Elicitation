...
for (month in months){
  ...
  experts_data_surveyed = fread(paste0(experts_path, '/Outputs/results_', month, '_cm.csv'))
...
#
  ...
  expert_nonrep_data = copy(model_data[type=='model',])
  expert_nonrep_data[, p_cm_val := NA]
  dim(expert_nonrep_data[, type := 'expert'])
  
  expert_data_expanded = rbind(experts_data, expert_nonrep_data[!(HZ %in% experts_data$HZ),])
  
  dim(expert_data_expanded)
  
  expert_data_expanded[HZ %in% experts_data$HZ,]
  
  expert_data_expanded[is.na(p_cm_val) & reported_cases == 1, p_cm_val:=0.05]
  expert_data_expanded[is.na(p_cm_val) & reported_cases == 0, p_cm_val:=0.00]
...
write.csv(expert_model_data_all, 'outputs/indevidual_results_with_scores_adj_additional.csv') 
