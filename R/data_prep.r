...
extract_totcase_data <- function(subarea = FALSE)
  ...
  DT_hz = DT[,max(confirmed_cases), by=ADM2_NAME]                                                       # calculate total cases to date by HZ
...
construct_time_series_mat <- function(DRC2_drc = extract_totcase_data())
  ...
  DT_hz = DT[,max(confirmed_cases), by=ADM2_NAME]
  DF_hz = as.data.frame.matrix(DT_hz)
  ...
  for (date in sort(unique(DRCDATAT$report_date) ))
  {
    ...
    tempframe = left_join(hznameframe, dplyr::select(DRCDATAT[DRCDATAT$report_date==date], "confirmed_cases", "ADM2_NAME"), by ="ADM2_NAME", name=character(date))
    casesbydate = append(casesbydate, c(as.vector(fix_nan(data.table::setorder(tempframe, "ADM2_NAME")$confirmed_cases))))
