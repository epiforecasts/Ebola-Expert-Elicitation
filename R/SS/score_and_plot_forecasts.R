  ...
  for (thresh in thresholds){
    ...
    for (e in experts){
      ...
      for (hz in sort(unique(experts_december[,'HZ']))){
        ...
        cases = as.double(actual_cases %>% filter(ADM2_NAME == str_to_title(hz)) %>% dplyr::select("cases_in_dec"))
        expert_scores = rbind(expert_scores,
                              data.frame(e = paste0(e), hz=hz, prob=prob, cases=cases, score, score_log, long_thresh, type='model', expert_date, forcast_to_date))
      }
      ...
      for (hz in sort(unique(experts_december[,'HZ']))){
        ...
        expert_scores = rbind(expert_scores,
                              data.frame(e='all', hz, probs=mean(probs), bincases[1], score, score_log,  long_thresh, type='expert', expert_date = nominal_forecast_date, forcast_to_date))
