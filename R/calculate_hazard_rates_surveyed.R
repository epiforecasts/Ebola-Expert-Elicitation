...
results_data[, nHZ := length(unique(HZ)), by=c('month')]
results_data[, nHZ_all := sum(unique(nHZ))]


results_data[, hazard_rate_gap := (predicted_hazard_rate - actual_hazard_rate)/nHZ]
results_data[, hazard_rate_gap_all := (predicted_hazard_rate_all - actual_hazard_rate_all)/nHZ_all]
...
ggsave('plots/bias_norm.pdf',hr_full, width=15, height=8)
ggsave('plots/bias_norm.png',hr_full, width=15, height=8, units= 'in')
